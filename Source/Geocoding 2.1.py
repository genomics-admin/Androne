    ...
    for index, item in enumerate(AltList):
        ...
        for index1, item1 in enumerate(AltList):
            if index<>index1:
                ...
                #print str(index)+'<->'+str(dist)+'<>'+str(index1)
                #-----1.5 is a fault tolarance limit, e.g. MtrLvl=10 then it will consider anypoint within 15 ft as nearby point. works fine upto 1 ft sample.
                if dist<(MtrLvl*1.5):  
                    #print str(index)+'<->'+str(dist)+'<>'+str(index1)
                    #val=val+','+str(index1)+'['+str(dist)+']'
                    val=val+','+str(index1)
