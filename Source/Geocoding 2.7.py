...
##LatLang.append('13.27233,77.31327')
##LatLang.append('13.27244,77.31304')
##LatLang.append('13.27222,77.31281')
##LatLang.append('13.2721,77.31287')

##LatLang.append('13.27617,77.30938')
##LatLang.append('13.27622,77.30881')
##LatLang.append('13.27646,77.30883')
##LatLang.append('13.27641,77.30935')
...
###Nandi Hills - Hilly region - non Sequential
LatLang.append('13.3547,77.68665')
LatLang.append('13.3554166666667,77.6869')
LatLang.append('13.3558,77.6856333333333')
LatLang.append('13.3549833333333,77.68525')
...
###Actual Field - STL
##LatLang.append('38.9456, -88.3995')
##LatLang.append('38.9482, -88.3995')
##LatLang.append('38.9482, -88.3988')
##LatLang.append('38.9456, -88.3988')
...
requestCounter=0
DetailsLevel = 0 #------was supposed to be used for increasing or decreasing details but the concept is not implemented yet
...
#------------------------------------------------------------
#------the process to validate and filter out all the
#------coordinates that don fall within the desired line
...
#------------------------------------------------------------
#------the process to calculate the distance between any 2
#------given coordinates
...
#------------------------------------------------------------
#------the superset process to find out the sample points----
...
#------------------------------------------------------------
#------the process that actually sends the request to google
#------server and filters the incoming response
...
#------------------------------------------------------------
#------to get the altitude of all the sample points----------
def AltiGater(LatLang,RequestLimit):
    ...
    return AltiStr
#------------------------------------------------------------
#------to find the center coordinate of the field------------
def findcentercoordinate(MainLatLangBkp,LatList,LngList):
    print 'Calculating center co-ordinate...                       ...[IN PROGRESS]'
    lat=[]
    lng=[]
    for index1,item1 in enumerate(MainLatLangBkp):
        lat2,lng2 = item1.split(',')
        lat.append(float(lat2))
        lng.append(float(lng2))
    return str(min(lat)+(max(lat)-min(lat))/2)+','+str(min(lng)+(max(lng)-min(lng))/2)
    print 'Calculating center co-ordinate...                       ...[DONE]'

#------------------------------------------------------------
#------this methode builds a KML compatible path between 2 given coordinates
def BuildKMLPathBetweenCoords(Coordinate1,Coordinate1Name,Coordinate1Desc,Coordinate2,Coordinate2Name,Coordinate2Desc,mode):
    print 'Building near-by Kml path design...                     ...[IN PROGRESS]'
    KmlPathStr=''
    print Coordinate1,Coordinate2
    timer=0
    center =""
    url='http://maps.googleapis.com/maps/api/directions/json?sensor=false&mode='+mode+'&origin='+Coordinate1+'&destination=' + Coordinate2
    ...
    j = json.loads(json_str)
    if j['status']=='OK':
        KmlDistanceStr=str(j['routes'][0]['legs'][0]['distance']['text'])+'('+str(j['routes'][0]['legs'][0]['duration']['text'])+')'       
        print KmlDistanceStr
    ...
    print 'Building near-by Kml path design...                     ...[Done]'    
    return KmlPathStr
#------------------------------------------------------------
#------get details about physical address and nearby locations
#------to build a path from the field to the nearby town/city/sub county
def findandbuildsurroundingaddress(centerCoordinate):
    print 'Fetching near-by locations data...                      ...[IN PROGRESS]'
    print centerCoordinate
    timer=0
    center =""
    url='http://maps.googleapis.com/maps/api/geocode/json?sensor=false&latlng=' + centerCoordinate
    ...
    j = json.loads(json_str)
    if j['status']=='OK':
        AddressStr=str(j['results'][0]['types'][0])+':'+str(j['results'][0]['formatted_address'])
        #str(j['results'][1]['types'][0])
        NearByLocDescStr=BuildKMLPathBetweenCoords(centerCoordinate,'','',str(j['results'][1]['geometry']['location']['lat'])+','+str(j['results'][1]['geometry']['location']['lng']),str(j['results'][1]['types'][0]),str(j['results'][1]['formatted_address']),'walking') 
    else:
        print "Data fetch failure. JSON didn't return OK.Raising error and aborting."
        #incase data query limit exceeded
        print(j['status'])
    ...
    #print NearByLocDescStr
    print 'Fetching near-by locations data...                      ...[DONE]'
    return AddressStr,NearByLocDescStr
#------------------------------------------------------------
#------the process that builds the KML file------------------
...
#------------sheet-----------------------------------------
    ...
    if SideWall==1:
        for index, item in enumerate(MainLatLangBkp):
            lat1,lng1 = item.split(',')
            for index1, item1 in enumerate(AltList):
                #print str(float(lat1))+'=='+str(LatList[index1])
                #print str(float(lng1))+'=='+str(LngList[index1])
                if str(float(lat1))==str(LatList[index1]) and str(float(lng1))==str(LngList[index1]):
                    #print str(LatList[index1])+','+str(LngList[index1])+','+str(AltList[index1])
...
#---------Total Info Display------------------------------------------------
#-----find the center point by looking for the point farthest from every main coordinate
    tempAltmax=[]
    for index, item in enumerate(MainLatLangAlt):
        lat1,lng1,alt1 = item.split(',')
        tempAltmax.append(float(alt1))
    centerCoordinate=findcentercoordinate(MainLatLangBkp,LatList,LngList)
    AddressStr,NearByLocDescStr=findandbuildsurroundingaddress(centerCoordinate)
    lat1,lng1 = centerCoordinate.split(',')
    f.write(MainCoordNoPlaceMarkDesignStart)
    f.write(FieldName)
    f.write(MainCoordNoPlaceMarkDesignMid1)
    f.write(AddressStr)
    ...
    f.write(str(lat1))
    f.write(MainCoordNoPlaceMarkDesignMid3)
    f.write(str(Alt_max+max(tempAltmax)+20))
    f.write(MainCoordNoPlaceMarkDesignMid4)
    f.write(str(lng1)+','+str(lat1)+','+str(Alt_max+max(tempAltmax)+35))
    f.write(MainCoordNoPlaceMarkDesignEnd)
    #print centerCoordinate
